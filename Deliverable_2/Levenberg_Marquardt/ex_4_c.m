...
for k = 1:nn
    ...
    gamma = find_gamma_armijo(@f, @compute_gradient, X(k, 1), Y(k, 1), d_k(1), d_k(2), s, beta, alpha);
...
fprintf(['Το ολικό ελάχιστο της συνάρτησης f χρησιμοποιώντας τη μέθοδο\n' ...
         'Levenberg-Marquardt, με δυναμικό βήμα Armijo και σημείο εκκίνησης το\n' ...
...
title('Σύγκλιση της Αντικειμενικής Συνάρτησης (Levenberg-Marquardt με Armijo)');
grid on;

figure;
plot(iterations, Eigenvalues(:, 1), '-o', 'LineWidth', 1.5, 'DisplayName', '\lambda_1');
hold on;
plot(iterations, Eigenvalues(:, 2), '-o', 'LineWidth', 1.5, 'DisplayName', '\lambda_2');
xlabel('Αριθμός Επαναλήψεων');
